...
for (int a = min_a; a <= max_a; a++)
    for (int b = (int)Math.sqrt((N-a*a)/3); b <= (int)Math.sqrt(N-a*a)+1; b++)
        for (int c = (int)Math.sqrt((N-a*a-b*b)/2); c <= (int)Math.sqrt(N-a*a-b*b)+1; c++){
              d = (int)Math.sqrt((N-a*a-b*b-c*c)/2);
			 if ((d*d)%P==N-(a*a)%P-(b*b)%P-(c*c)%P){
